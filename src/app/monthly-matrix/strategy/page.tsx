...
export default function MonthlyStrategyWorkspacePage() {
  ...
  const approved = strategy?.status === "approved" || strategy?.status === "partially_approved";
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={embedded ? "确认目标问题、内容类型组合、渠道配额和资料版本；保存后在当前页面继续生产预检。" : "确认目标问题、内容类型组合、渠道配额和资料版本；保存后回到矩阵运行生产预检。"}
        actions={embedded ? undefined : <Space wrap><Link href="/monthly-matrix"><Button icon={<ArrowLeftOutlined />}>返回月度内容矩阵</Button></Link><Link href="/monthly-matrix/content-types"><Button icon={<BookOutlined />}>管理内容类型</Button></Link></Space>}
      />
