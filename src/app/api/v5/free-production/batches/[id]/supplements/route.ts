...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await supplementFreeProductionBatch(params.id, { ...readFreeProductionMutation(payload), supplements: Array.isArray(payload.supplements) ? payload.supplements as Array<{ riskId: string; value: string | { fileName: string; mimeType: string; dataBase64: string } }> : [] }, request.headers.get("x-idempotency-key"));
