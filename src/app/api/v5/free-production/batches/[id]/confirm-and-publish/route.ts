...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await confirmAndPublishFreeProductionBatch(params.id, { ...readFreeProductionMutation(payload), contentDigest: typeof payload.contentDigest === "string" ? payload.contentDigest : "" }, request.headers.get("x-idempotency-key"));
