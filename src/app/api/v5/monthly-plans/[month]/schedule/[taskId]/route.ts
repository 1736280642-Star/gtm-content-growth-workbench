...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ month: string; taskId: string }> }) {
  const routeParams = await params;
  try {
    const data = await scheduleV5ProductionTask(routeParams.month, routeParams.taskId, parseScheduleTaskRequest(await request.json()));
