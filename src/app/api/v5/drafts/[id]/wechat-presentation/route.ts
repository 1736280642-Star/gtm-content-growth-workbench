...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await getWechatPresentationState(routeParams.id);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await generateWechatPresentation({ draftVersionId: routeParams.id, approvedImageRoles: body.approvedImageRoles, coverImageRef: body.coverImageRef, actor });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await decideWechatPresentation({ draftVersionId: routeParams.id, artifactId, decision, reason, actor });
    return NextResponse.json({ ok: true, data });
  } catch (error) {
    return singleArticleErrorResponse(error);
  }
