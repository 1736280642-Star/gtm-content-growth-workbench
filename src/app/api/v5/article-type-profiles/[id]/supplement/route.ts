...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await supplementArticleTypeProfile(routeParams.id, parseArticleTypeSupplementRequest(await request.json()), request.headers.get("x-idempotency-key"));
