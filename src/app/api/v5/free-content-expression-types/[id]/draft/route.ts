...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await createFreeContentExpressionTypeVersion(routeParams.id, { ...readFreeProductionMutation(payload), input: payload.input as FreeContentExpressionTypeDraftInput }, request.headers.get("x-idempotency-key"));
