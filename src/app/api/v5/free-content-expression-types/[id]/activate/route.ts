...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { return NextResponse.json({ ok: true, data: await activateFreeContentExpressionType(routeParams.id, readFreeProductionMutation(await readFreeProductionPayload(request)), request.headers.get("x-idempotency-key")) }); }
