...
import { Button, Divider, Input, InputNumber, Spin, Switch } from "antd";
...
export default function HostedTaskPage() {
  ...
  async function demoLogin() {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(readApiError(payload, "演示登录失败。"));
      ...
      <div className={styles.workspace}>
        <div className={styles.formColumn}>
          <section className={styles.section} id="setup-identity">
            ...
            {identityStatus === "checking" ? <div className={styles.inlineLoading}><Spin size="small" /> 正在检查当前登录状态</div> : identity ? <div className={styles.accountRow}><div className={styles.accountIdentity}><span className={styles.mailMark}><UserOutlined /></span><span><strong>{identity.email}</strong><span>{roleLabels[identity.role]} · 登录已验证</span></span></div><Button type="text" icon={<LogoutOutlined />} onClick={logout}>退出或更换账号</Button></div> : loginSent ? <div className={styles.loginSentCard}><CheckCircleFilled /><div><strong>登录邮件已发送到 {loginEmail}</strong><span>请在 15 分钟内打开邮件链接。完成后会自动回到这里。</span></div><Button onClick={() => setLoginSent(false)}>更换邮箱或重新发送</Button></div> : <div className={styles.inlineLoginStack}>{DEMO_MODE ? <div className={styles.demoLoginEntry}><Button type="primary" size="large" icon={<ThunderboltOutlined />} loading={loginSending} onClick={demoLogin}>一键进入已跑通的演示工作台</Button><Divider plain>或体验邮箱登录</Divider></div> : null}<div className={styles.inlineLoginForm}><label><span>工作邮箱</span><Input size="large" prefix={<MailOutlined />} value={loginEmail} onChange={(event) => setLoginEmail(event.target.value)} onPressEnter={requestLogin} placeholder="name@company.com" /></label><Button type="primary" size="large" icon={<ArrowRightOutlined />} loading={loginSending} onClick={requestLogin}>发送登录链接</Button></div></div>}
          </section>
