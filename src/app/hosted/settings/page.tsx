...
export default function HostedSettingsPage() {
  ...
  return (
    <div className={styles.settingsPage}>
      ...
      <section className={styles.settingsCard}><div className={styles.settingsTitle}><MailOutlined /><div><strong>结果通知</strong><span>需要你操作的策略、样文和异常邮件不能关闭。</span></div></div><div className={styles.switchRows}><label><span><strong>每日 URL 汇总</strong><small>当日批次关闭后发送一封</small></span><Switch checked={dailyDigest} onChange={setDailyDigest} /></label><label><span><strong>本轮完成通知</strong><small>复盘结果形成后发送</small></span><Switch checked={monthlyCompleted} onChange={setMonthlyCompleted} /></label></div></section>
