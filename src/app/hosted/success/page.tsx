...
type HostedStatus = "preparing" | "pending_strategy_review" | "pending_sample_review" | "running" | "action_required" | "paused" | "completed";
...
export default function HostedSuccessPage() {
  ...
  const loadOrder = useCallback(async (targetOrderId: string) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "托管回执读取失败。请稍后重试。");
    } finally {
      setLoading(false);
    }
  }, []);
