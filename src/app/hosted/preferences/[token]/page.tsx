...
export default function HostedPreferencesPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <div className={styles.settingsPage}>
      ...
      <section className={styles.settingsCard}><div className={styles.settingsTitle}><MailOutlined /><div><strong>可选结果通知</strong><span>设置立即生效，后续邮件中的链接也可以再次修改。</span></div></div><div className={styles.switchRows}><label><span><strong>每日 URL 汇总</strong><small>当日批次关闭后发送一封；关闭相当于退订每日结果</small></span><Switch checked={dailyDigest} onChange={setDailyDigest} /></label><label><span><strong>本轮完成通知</strong><small>本轮发布任务全部收口并完成复盘后发送</small></span><Switch checked={monthlyCompleted} onChange={setMonthlyCompleted} /></label><label><span><strong>必须处理的行动邮件</strong><small>策略、样文、授权失效和阻断，不能关闭</small></span><Switch checked disabled /></label></div></section>
