      ...
      {artifact ? (
        <>
          ...
          <div className="generation-result-columns">
            <WechatArticlePreview artifact={artifact} productId={batch.productId} batchId={batch.id} batchVersion={batch.version} coverRisk={batch.risks.find((risk) => risk.key === "wechat_cover")} changingLayout={working === "layout"} savingContent={working === "content"} savingCover={working === "cover"} locked={["publishing", "draft_created", "published", "cancelled"].includes(batch.status)} onChangeLayout={(templateId) => onChangeLayout(artifact.id, templateId)} onEditContent={(input) => onEditContent(artifact.id, input)} onBindVisual={(suggestionId, mediaAssetId) => onBindVisual(artifact.id, suggestionId, mediaAssetId)} onSaveCover={onSaveCover} />
            {step === "sources" ? <CitationSourcePanel sources={artifact.sourceExcerpts} sections={artifact.sections} hotspotPanel={<HotspotSidebarPanel plan={artifact.hotspotIntegration} hasPreviousVersion={Boolean(artifact.previousArtifactId)} integrating={working === "hotspot"} restoring={working === "restore"} locked={["publishing", "draft_created", "published", "cancelled"].includes(batch.status)} error={hotspotError} onIntegrate={() => onIntegrateHotspot(artifact.id, artifact.hotspotIntegration ? "replace" : "integrate")} onRestore={() => onRestorePreviousVersion(artifact.id)} />} onContinue={() => setStep("publish")} /> : <aside className="publication-prep-panel"><Button type="text" icon={<ArrowLeftOutlined />} onClick={() => setStep("sources")}>返回引用来源</Button><RiskAndGapPanel risks={batch.risks} saving={working === "supplements"} onSubmit={onSupplements} />{batch.channelConfig.channel !== "wechat_official_account" ? <ConfirmAutoPublishButton batch={batch} loading={working === "publish"} onConfirm={onPublish} /> : null}</aside>}
          </div>
        </>
