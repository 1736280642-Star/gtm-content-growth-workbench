...
const statusLabels: Record<string, { label: string; color?: string }> = {
  ...
  draft_created: { label: "已写入草稿箱", color: "success" },
...
export function FreeProductionTaskTable({ data, loading, selectionMode, selectedRowKeys, onSelectionChange, onPreview, onRetry }: { data: FreeProductionBatch[]; loading?: boolean; selectionMode?: boolean; selectedRowKeys?: string[]; onSelectionChange?: (ids: string[]) => void; onPreview: (batch: FreeProductionBatch) => void; onRetry: (batch: FreeProductionBatch) => void }) {
  ...
  return <Table
    ...
    rowSelection={selectionMode ? {
      ...
      getCheckboxProps: (record) => ({
        disabled: ["publishing", "draft_created", "published"].includes(record.status),
        title: ["publishing", "draft_created", "published"].includes(record.status) ? "正在发送或已经写入渠道的文章需保留发布记录" : undefined
      })
    } : undefined}
    rowClassName={(record) => selectedRowKeys?.includes(record.id) ? "free-task-row-selected" : ""}
    pagination={{ pageSize: 12 }}
    locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="还没有公众号生产批次" /> }}
  />;
}
