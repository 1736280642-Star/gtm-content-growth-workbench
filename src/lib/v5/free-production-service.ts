...
import type { DirectPublishPlatformKey, WorkspaceRole } from "@/lib/types";
...
const PUBLISH_ROLES = new Set<WorkspaceRole>(["workbench_operator", "developer_admin"]);
...
function actor(roles = WRITE_ROLES) {
  const value = readWorkbenchState().workspaceSetting.currentRole;
  if (!roles.has(value)) throw new FreeProductionServiceError(403, "FREE_PRODUCTION_FORBIDDEN", "当前角色无权执行此操作。", "切换到具备内容生产或发布权限的角色后重试。");
  return value;
}
...
function catalogProducts(): FreeProductionCatalogProduct[] {
  ...
  return state.knowledgeBases.filter((knowledgeBase) => knowledgeBase.productionStatus === "ready").map((knowledgeBase) => {
    ...
    return {
      ...
      name: productName === "腾讯云 ADP" ? "JOTO 腾讯云 ADP 实施服务" : productName,
...
function selectionFor(expression: FreeContentExpressionTypeVersion, catalog: FreeProductionCatalog) {
  const product = catalog.products.find((item) => item.productId === expression.productId);
  const issues: string[] = [];
  if (!product) issues.push("表达绑定的产品未进入当前生产池。");
  const rulePackage = product?.rulePackages.find((item) => item.status === "active");
  if (!rulePackage) issues.push("表达绑定产品缺少已激活的产品表达规则包。");
  const availableSnapshots = new Set(product?.knowledgeBases.map((item) => item.sourceSnapshotId) || []);
  if (!expression.knowledgeSnapshotIds.length || expression.knowledgeSnapshotIds.some((id) => !availableSnapshots.has(id))) issues.push("表达绑定的知识快照不存在、不可用或与产品不一致。");
  if (!expression.applicableChannels.includes(expression.channelBinding.channel)) issues.push("表达渠道绑定与适用渠道不一致。");
  if (expression.publishPolicy !== "automatic_after_confirmation") issues.push("表达未绑定统一自动发布策略。");
  return { product, rulePackage, issues };
}

function knowledgeFor(expression: FreeContentExpressionTypeVersion, product: FreeProductionCatalogProduct) {
  const foundation = readV5FoundationState();
  return product.knowledgeBases.filter((item) => expression.knowledgeSnapshotIds.includes(item.sourceSnapshotId)).map((item) => ({
...
function generationPrompt(input: { batch: FreeProductionBatch; expression: FreeContentExpressionTypeVersion; knowledge: Array<Record<string, unknown>>; brandBaseline: Record<string, unknown>; affectedSectionKeys?: string[]; currentArtifact?: ContentDraftArtifact }) {
  ...
  return {
    ...
    userPrompt: JSON.stringify({
      ...
      productName: input.batch.productName,
...
async function generateArtifact(input: { batch: FreeProductionBatch; expression: FreeContentExpressionTypeVersion; knowledge: Array<Record<string, unknown>>; brandBaseline: Record<string, unknown>; affectedSectionKeys?: string[]; currentArtifact?: ContentDraftArtifact }) {
  ...
  let response = await callAiProvider({ provider: "qwen", ...prompt, temperature: 0.25 });
  ...
  let validation = validateFreeProductionOutput({ expression: input.expression, productName: input.batch.productName, ...parsed });
  let repairCount: 0 | 1 = 0;
  if (validation.repairableIssues.length && !validation.blockingIssues.length) {
    response = await callAiProvider({ provider: "qwen", systemPrompt: prompt.systemPrompt, userPrompt: `${prompt.userPrompt}\n\n上次输出未通过确定性检查：${validation.repairableIssues.join("；")}。只修复这些结构与表达问题，仍输出完整 JSON。`, temperature: 0.15 });
    repairCount = 1;
    if (response.ok && response.content) {
      try { parsed = parseProviderJson(response.content); } catch { /* Preserve the last parse for actionable failure reporting. */ }
      validation = validateFreeProductionOutput({ expression: input.expression, productName: input.batch.productName, ...parsed });
    }
  }
  const selectedTitle = parsed.titleCandidates[0] || `${input.batch.productName}：一段真实工作流的变化`;
...
async function runGeneration(batchId: string, options?: { affectedSectionKeys?: string[]; auditReason?: string; actorId?: string }) {
  ...
  const selection = selectionFor(expression, catalog);
  if (!selection.product || selection.issues.length) throw new FreeProductionServiceError(422, "FREE_PRODUCTION_CONFIG_INVALID", "当前表达暂时不可生产。", "返回表达列表选择其他表达；管理员修复配置。", selection.issues);
  const knowledge = knowledgeFor(expression, selection.product);
  ...
  const supplementalFacts = Object.fromEntries(batch.risks.filter((risk) => risk.status === "ready" && risk.value).map((risk) => [risk.key, risk.value!]));
  const plan = compileExpressionPlan({ batchId, expression, knowledgeSnapshots: knowledge, supplementalFacts, previousPlan });
  const inputSnapshot = {
  ...
  const generated = await generateArtifact({ batch: { ...batch, inputSnapshots: [...batch.inputSnapshots, inputSnapshot] }, expression, knowledge, brandBaseline, affectedSectionKeys: options?.affectedSectionKeys, currentArtifact });
  if (!generated.ok) {
...
export async function createFreeProductionFromExpression(input: { expectedVersion: number; auditReason: string; expressionTypeVersionId: string }, header: string | null) {
  ...
  const catalog = await getFreeProductionCatalog(); const selection = selectionFor(expression, catalog);
  if (!selection.product || !selection.rulePackage || selection.issues.length) throw new FreeProductionServiceError(422, "FREE_PRODUCTION_CONFIG_INVALID", "当前表达暂时不可生产。", "返回表达列表选择其他表达；管理员修复配置。", selection.issues);
  ...
  const creation = await updateFreeProductionState((state) => {
    const replay = replayBatch(state, context.key, input);
    if (replay) return { batch: replay, created: false };
    const now = new Date().toISOString(); const id = `free-batch-${randomUUID()}`; const risks = createInitialRisks(expression);
    const value: FreeProductionBatch = { id, monthlyPlanId: monthlyPlan.id, ...bounds, productId: expression.productId, productName: selection.product!.name, productExpressionRulePackageVersionId: selection.rulePackage!.id, knowledgeSnapshotIds: expression.knowledgeSnapshotIds, freeContentExpressionTypeVersionId: expression.freeContentExpressionTypeVersionId, supplementalMaterialRefs: [], riskAndGapSummary: summarizeRisks(risks), channelConfig: expression.channelBinding, publishPolicy: "automatic_after_confirmation", status: "compiling", repairCount: 0, risks, expressionPlans: [], inputSnapshots: [], draftArtifacts: [], idempotencyKey: context.key, createdBy: actorId, createdAt: now, updatedAt: now, version: 1 };
    ...
    state.audits.push({ auditId: randomUUID(), action: "free_production_created_from_expression", objectId: id, actor: actorId, auditReason: context.auditReason, createdAt: now, summary: { expressionTypeVersionId: expression.freeContentExpressionTypeVersionId, monthlyPlanId: monthlyPlan.id } });
...
export async function confirmAndPublishFreeProductionBatch(batchId: string, input: { expectedVersion: number; auditReason: string; contentDigest: string }, header: string | null) {
  const actorId = actor(PUBLISH_ROLES); const context = mutationContext(input, header); const requestPayload = { batchId, ...input };
