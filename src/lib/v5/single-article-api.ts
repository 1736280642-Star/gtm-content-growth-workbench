...
export function getSingleArticleActor(): SingleArticleActor {
  ...
  if (process.env.NODE_ENV === "production" && !enabled) {
    throw new SingleArticleProductionError(503, "authorization_not_configured", "正式内容写接口尚未启用可信服务端身份。", "配置服务端身份与 V5_SINGLE_ARTICLE_WRITES_ENABLED 后重试。");
  }
  ...
  if (!actorId || !actorRole) {
    throw new SingleArticleProductionError(503, "actor_not_configured", "正式内容操作缺少可信操作者或允许角色。", "配置 V5_SINGLE_ARTICLE_ACTOR_ID 与 workbench_operator/developer_admin 角色。");
  }
  return { actorId, actorRole, actorType: "human", auditReason: "用户从批量生成中心发起正式内容生成" };
}
...
export function singleArticleErrorResponse(error: unknown) {
  ...
  return NextResponse.json({ ok: false, error: { code: "single_article_internal_error", message: "正式内容请求处理失败。", nextAction: "根据关联 ID 查看服务端日志后重试。" } }, { status: 500 });
}
