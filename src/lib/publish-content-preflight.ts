...
export const PUBLISH_PREFLIGHT_RULE_VERSION = "2026-08-13.2";
...
const directPromotionPattern = /扫码|二维码|加微信|私信我|立即购买|限时(?:优惠|活动|折扣|抢购|秒杀|福利|免费)|免费领取|咨询我们|联系我们|点击下单|官网购买/gi;
const technicalHeadingPattern = /(^|\n)#{1,4}\s*(实现|实施|原理|配置|代码|验证|测试|排查|步骤|架构|接口|部署)/gim;
...
export function preflightPublishContent(input: {
  ...
  const promotionMatches = countMatches(`${title}\n${markdown}`, directPromotionPattern);
  const technicalSignals =
    countMatches(markdown, technicalHeadingPattern) +
  ...
  if (input.platform === "juejin") {
    if (plain.replace(/\s/g, "").length < 600) blockers.push({ code: "juejin_depth_too_low", message: "Juejin content must contain at least 600 non-space characters." });
    if (technicalSignals < 2) blockers.push({ code: "juejin_technical_signals_missing", message: "Juejin content needs at least two implementation, code, verification, or step signals." });
    if (externalLinkCount > 2) blockers.push({ code: "juejin_external_links_excessive", message: "Juejin content contains more than two external links." });
    if (promotionMatches > 0) blockers.push({ code: "juejin_promotion_risk", message: "Juejin content contains direct promotional or contact language." });
  } else {
  ...
  return {
    ...
    blockers,
    warnings,
    scores: { contentLength: plain.replace(/\s/g, "").length, technicalDepth: technicalSignals, promotionRisk: promotionMatches, externalLinkCount },
    checkedAt: input.checkedAt || new Date().toISOString()
  };
}

export function rewriteJuejinContentOnce(input: { title: string; markdown: string }) {
  const title = input.title.replace(/[！？?]{2,}/g, "").replace(/(限时|免费领取|立即购买|重磅)/g, "").trim().slice(0, 80);
  const lines = input.markdown
    .split(/\r?\n/)
    .filter((line) => {
      directPromotionPattern.lastIndex = 0;
      return !directPromotionPattern.test(line);
    })
    .map((line) => line.replace(/\[([^\]]+)]\(https?:\/\/[^)]+\)/g, "$1"));
  let markdown = lines.join("\n").replace(/\n{3,}/g, "\n\n").trim();
  technicalHeadingPattern.lastIndex = 0;
  if (!technicalHeadingPattern.test(markdown)) markdown = `## 问题与实现思路\n\n${markdown}`;
