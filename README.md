...
- **渠道规则包**：平台收录规则（域名、板块、结构要求、对比维度、CTA 变体引用）以受治理的版本化配置注入（环境变量 `GEO_CHANNEL_RULE_PACK_JSON`），领域代码不硬编码平台域名与文案；规则包缺省时链路以“无平台感知”模式兼容运行，配置非法则 fail-closed；
- **平台感知查询**：调研查询按内容类型证据需求编译——平台收录格局（`site:{domain}` 约束查询）、踩坑证据、开源自建替代、量化成效基准、采购误区五类查询意图；补充轮按证据缺口定向补查（缺平台证据补 `site:` 查询、缺独立来源补变体查询）；
...
- **提及率 KPI 闭环**：每次调研固化 `mention_baseline`（提及率基线 + 渠道引用统计，确定性推导不经 LLM）；批准蓝图发布后自动触发 `post_publish_retest` 复测 run（探针集 = 蓝图复测问题），确定性计算提及率 delta（新增/失去提及问题、提及率增量）；达标进入监控，未达标自动触发下一轮调研改进策略；
