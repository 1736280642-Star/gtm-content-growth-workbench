...
| V5 MySQL Schema cutover | 已完成 | 原生 V5 月度表已建立；V4 周度关系表已从 MySQL Schema 删除 |
...
| RAG / EvidencePack / Final Evidence Gate | 设计完成，生产实现未完成 | 不能把 EvidencePreview 或历史 Chunk 直接当正式生成许可 |
...
注意：V5 cutover 脚本包含删除旧 V4 周度关系表的 migration。只有在确认目标数据库是新建 V5 数据库，或已经完成备份和人工审批后，才能使用带删除确认的执行参数。不要对来源不明或仍承载业务数据的数据库直接执行 destructive migration。
