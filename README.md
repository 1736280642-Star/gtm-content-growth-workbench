...
| 日期执行、正式发布与数据回传 | 已提供微信官方 API、CSDN/掘金/知乎本机 Arcs runner、状态验证、URL 回填和指标导入 | 代码具备执行链路；仍需逐账号单篇验收，HTTP 200 或按钮点击不等于公开发布成功 |
...
CTA 不由模型自由选择。系统根据目标产品、渠道、CTA 意图、内容类型、标题类别、推广目标、审批状态、有效期和优先级生成冻结 `CTAPlan`。同业务优先级存在多个候选时阻断；必需 CTA 无匹配时阻断；非必需 CTA 无匹配时生成无 CTA 正文。

通用代码不写死 WorkBuddy、ADP 或其他产品的身份句、CTA 文案和 URL。正文事实只能来自冻结的 `FinalEvidencePack`；输出校验覆盖标题、篇幅、结构、事实追踪、适用边界、CTA、URL、敏感信息、重复段落和跨渠道相似度。

当前边界：确定性解析、合同编译、输出校验和一次修复已经有独立领域实现与测试；MySQL `PromotionProfileVersion` 仓库、配置审批 UI，以及正式批量生成链路对该合同的调用仍需后续接线。详细方案见 `docs/V5-07-20/内容生产规则链路与动态推广确定性解析方案.md`。
...
- 微信官方正式发布 bridge、四平台幂等排程和 CSDN/掘金/知乎本机 Arcs runner。
...
| `arcs-runner/` | 仅监听本机的 CSDN、掘金、知乎正式发布执行器 |
