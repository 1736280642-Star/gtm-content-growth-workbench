...
if (!sourcePath || !outputDirectory) {
  console.error("Usage: node scripts/extract-wechat-article-layout.mjs <source-html> <output-directory>");
  process.exit(1);
}
...
const account = source.match(/<a[^>]*id=["']js_name["'][^>]*>([\s\S]*?)<\/a>/i)?.[1]?.trim() ?? "";
const contentMatch = source.match(
  /<div([^>]*\bid=["']js_content["'][^>]*)>([\s\S]*?)<\/div>\s*<script[^>]*>[\s\S]*?var\s+first_sceen__time/i
);

if (!contentMatch) throw new Error("Unable to find the WeChat article body (#js_content).");
...
const content = rawContent.replace(/(?<!\d)(1[3-9]\d)\d{4}(\d{4})(?!\d)/g, "$1****$2");
