...
| 7. 月度复盘只有数据没有判断 | 复盘如果只列指标，不能反哺下月选题 | 月度复盘必须输出“继续写/减少写/补强写”的建议 |
...
| 月度复盘导出 | Markdown 优先，后续支持 PDF/Word |
...
| 月度计划 | `POST /monthly-plans/generate`、`PATCH /monthly-plans/{id}` |
...
| 月度复盘 | `GET /monthly-reviews/{month}` |
...
| API 可用性 | 首页概览、月度计划、任务、发布队列至少有基础接口 |

## 9. Phase 2：月度计划与任务流

### 9.1 目标

跑通从工作台设置到月度计划，再到每日任务的流程。
...
| 验收项 | 标准 |
|---|---|
| 月度复盘生成 | 能生成管理层摘要和运营执行明细 |
| 渠道复盘 | 能看到各渠道发布和表现 |
| 博客诊断 | 能看到 SEO/GEO 问题汇总 |
| GEO 复盘 | 能看到各平台提及和引用情况 |
| 下月建议 | 能输出具体选题方向 |
| 候选池 | 可将建议主题加入博客候选池 |
| 导出 | 支持导出 Markdown 月度复盘 |
