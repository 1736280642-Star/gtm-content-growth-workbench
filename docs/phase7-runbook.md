...
20. 周计划页已支持单条确认、批量确认和删除未生成任务。
21. 周度复盘页已支持导出 Markdown，并通过 `GET /api/weekly-reports/{week}/export` 返回可交付周报文本。
22. 周度复盘页已支持把周报建议生成下周计划草稿，并通过 `POST /api/weekly-reports/{week}/next-plan` 写入运行态。
23. GEO 测试页已支持把未命中或官网链路不足结果加入博客候选池，并通过 `POST /api/geo-test-results/{id}/candidate` 写入运行态。
...
26. 博客候选池页已支持生成渠道补强任务，并通过 `POST /api/blog-articles/{id}/candidate/task` 追加到当前周计划。
27. 周度复盘页已展示渠道 URL/指标、官网博客诊断明细和 GEO 测试明细，未生成周报时也会先展示当前运行态数据。
28. 新增 `smoke:browser`，通过系统 Chrome 真实点击周计划 Popconfirm、知识库新增 Modal 和发布 URL 回填 Modal，并验证刷新后的 DOM 状态。
29. 博客监控页已补候选状态、优先级、建议原因和下一步判断，可直接区分先诊断、建议入候选池、候选池处理、已规划、继续观察和暂不处理。
30. GEO 测试页已补问题级别、候选状态、建议原因和下一步判断，可直接区分配置模型、排查失败、建议入候选池、补官网引用、候选池处理和继续观察。
31. 周计划页已补稿件承接、发布承接和下一步判断，可直接区分待确认、待生成、终稿确认、人工发布、URL 回填、录入指标和可复盘。
32. 首页已补执行队列，可把周计划、稿件、发布、博客、GEO 和周报复盘的待处理项汇总成优先动作入口。
...
36. 周计划页已补“处理动作 / 可执行入口 / 维护”，把计划确认、稿件生成、AI 配置排查、终稿处理、发布回填、指标录入和复盘入口拆成单行闭环。
37. 首页执行队列已补“当前状态 / 处理动作 / 可执行入口”，负责人可直接从首页判断当前阻塞项和下一步跳转。
38. 周报页复盘行动队列已把“当前动作”统一为“处理动作”，全站关键业务页表达已收敛到同一套闭环语言。
...
41. 周报页的下周建议已补成“建议 / 下一步 / 处理动作 / 可执行入口”，未生成周报时先收束到生成周报，已生成周报时可直接复核建议并生成下周计划草稿。
...
GET /api/weekly-reports/{week}/export
POST /api/weekly-reports/{week}/next-plan
POST /api/geo-test-results/{id}/candidate
...
8. `worker:sync-blog`、`worker:run-geo-tests`、`worker:import-log`、`worker:import-channel-metrics` 可通过本地 API 执行真实任务。
...
21. 页面失败态已覆盖首页、周计划、今日任务、终稿确认、发布队列、博客监控、博客候选池、GEO、周报、知识库、真实接入和设置页。
22. `smoke:pages` 已覆盖主要页面访问、运行态 API、配置诊断 API、周报 API 和周报 Markdown 导出 API。
23. `smoke:interactions` 已覆盖 64 个关键页面动作的源码交互合约。
24. `smoke:workflow` 已覆盖周报 Markdown 导出、下周计划草稿生成、GEO 结果入候选池、博客候选池生命周期、发布记录单条指标回填和博客候选生成渠道任务，当前通过结果为 36/36。
25. 周报页已补齐渠道复盘、博客诊断复盘和 GEO 测试明细展示，结构验证当前通过结果为 238/238。
...
30. GEO 测试页已支持按平台、Prompt 组、执行状态、JOTO 提及、官网引用和数据来源筛选，并在筛选无结果时提供清空筛选动作。
...
36. 周报页已支持按发布状态、博客 GEO 结果和 GEO 执行状态筛选复盘明细，并在筛选无结果时提供清空筛选动作。
37. 周计划页已支持按任务状态、渠道和产品筛选任务，并在筛选无结果时提供清空筛选动作。
...
43. GEO 测试页已展示问题级别、候选状态、建议原因和下一步执行判断，能直接区分配置模型、排查失败、建议入候选池、补官网引用、候选池处理和继续观察。
44. 周计划页已展示稿件承接、发布承接和下一步执行判断，能从计划入口直接追踪任务是否生成、质检、入队、发布、回填和复盘。
...
48. 真实接入页已展示接入缺口和可执行入口，能从交接页直接进入 AI 配置、GEO 测试、博客同步或日志导入。
49. 博客候选池页已展示候选处置判断和处理动作，能从候选池直接判断该确认入池、生成任务、标记规划、复查来源，还是去周计划回看承接结果。
50. 周报页已展示复盘行动队列，能从复盘页直接判断该处理发布、回填 URL、录入指标、处理博客候选、排查 GEO 配置、沉淀 GEO 候选，还是生成下周计划。
51. 今日任务页已展示执行承接判断，能从当天任务直接判断该回周计划确认、生成稿件、排查生成、处理质检、终稿确认、进入发布队列、回填 URL、录入指标，还是去周报复盘。
52. 终稿确认页已展示执行承接判断，能从终稿页直接判断该检查生成配置、重新生成、处理质检阻断、加入发布队列、去发布页承接、回填 URL、录入指标，还是去周报复盘。
53. 博客监控页已展示处理动作和可执行入口，能从博客列表直接判断该先诊断、加入候选池、去候选池处理、看周计划、继续观察，还是进入周报复盘。
...
1. 周计划页：`生成周计划` 调用 `POST /api/weekly-plans/generate`。
...
8. GEO 页：`批量运行测试` 调用 `POST /api/geo-tests/run`，缺配置时显示 `pending_config`。
9. 博客监控页：`同步博客内容` 调用 `POST /api/blog-articles/sync`，缺配置时显示 `pending_config`。
10. 周计划页：`编辑` 调用 `PATCH /api/content-tasks/{id}`。
11. 周计划页：`重生成` 调用 `POST /api/content-tasks/{id}/regenerate-title`。
12. 周计划页：`确认` / `批量确认` 调用 `POST /api/content-tasks/confirm`。
13. 周计划页：`删除` 调用 `DELETE /api/content-tasks/{id}`，只允许删除未生成稿件的任务。
14. 终稿页：`重新生成` 调用 `POST /api/content-tasks/{id}/generate`。
15. 发布页：`标记已发布` 调用 `PATCH /api/publish-records/{id}/published`。
16. GEO 页：`查看快照` 显示原始回答，`人工修正` 调用 `PATCH /api/geo-test-results/{id}/override`。
...
24. 周报页：`导出 Markdown` 调用 `GET /api/weekly-reports/{week}/export`，并复制可交付周报文本。
25. 周报页：`生成下周计划草稿` 调用 `POST /api/weekly-reports/{week}/next-plan`，并把周报建议反哺到下一轮排期。
26. GEO 页：`入候选池` 调用 `POST /api/geo-test-results/{id}/candidate`，把未命中或链路不足主题沉淀到博客候选池。
...
29. 博客候选池页：`生成任务` 调用 `POST /api/blog-articles/{id}/candidate/task`，把博客/GEO 缺口转成当前周计划的渠道补强任务。
30. 周报页：展示渠道发布 URL、`channelMetrics`、博客 `blogDiagnostics` 和 GEO `geoResults` 明细，用于从复盘直接判断下周补强方向。
31. 周报页：新增“复盘行动队列”，可把发布队列、URL 回填、渠道指标、博客候选、GEO 配置和 GEO 缺口聚合成可执行动作，并直接跳转到处理页面。
32. GEO 页：新增“处理动作 / 可执行入口”，把配置排查、失败快照、候选池承接、周计划回看和周报复盘串成可点击闭环。
...
36. 设置页：规则检查新增“可执行入口”，把规则修正、真实日志接入、GEO 测试和周计划承接串到同一张表里。
...
3. GEO 测试已接入 AI Provider 路径；缺配置时返回 `pending_config`，不生成假 GEO 结果。
...
再验证。`smoke:browser` 和 `smoke:workflow` 都会写入 `data/workbench-state.json`，需要顺序执行，不能并行跑；否则浏览器 smoke 准备好的任务可能被 workflow 重建周计划覆盖，导致误报：
