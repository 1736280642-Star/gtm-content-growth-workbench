...
2. 让内容生产不再靠临时判断，而是由知识库、蒸馏词、渠道表现、GEO 测试和周度复盘共同驱动。
3. 让用户每天看到的是明确任务，每周看到的是增长复盘，每月看到的是认知资产沉淀。
...
4. 自动把 GEO、博客监控、渠道数据转成候选主题和周计划信号。
...
| 场景 | 用户目标 | 工作台应承担的工作 |
|---|---|---|
| 周初规划 | 确认本周要写什么 | 根据周报问题、GEO 缺口、渠道数据、蒸馏词矩阵自动生成计划预览 |
| 日常生产 | 今天要发哪些内容 | 从已确认周计划中选择任务，批量生成正文 |
...
| GEO 复盘 | 判断 AI 是否推荐 JOTO | GEO 测试页从结果表格升级为诊断复盘台 |
...
2. 没有清晰展示本周从计划到生成、发布、回填、数据回传的进度。
...
1. 周计划仍然带有“进入今日任务生成队列”“覆盖草稿和发布队列”等旧逻辑。
2. 周计划和正文生成边界不够清楚。
3. 周计划生成规则如果只按优先级排序，会把产品推广和品牌长期表达放到后面，这是错误的。
4. 周计划预览字段应轻量，但当前状态字段和后续发布状态过多。

V3 方向：

1. 周计划只做标题级计划预览，不生成正文，不展示草稿结构。
2. 周计划预览字段只保留：
   - 标题
   - 渠道
   - 产品
...
3. 缺少顶部“今日计划、今日已发布、本周剩余”的执行进度。
4. 用户需要先从周计划选择今天要生成哪些文章，再批量生成，而不是默认所有任务都混在一个表格里。
...
5. 官网引用改为四层指标：
   ...
   - 非官方引用
6. GEO 测试频率按发布量区间推荐：
   - 每周发布量 <= 5 篇：每月 1 次完整 GEO 测试
   - 每周发布量 6 到 15 篇：每两周 1 次完整 GEO 测试
   - 每周发布量 >= 16 篇：每周 1 次完整 GEO 测试
7. 额外触发：
   - 官网上新一批博客
   - 官网首页或产品页改版
   - 新蒸馏词矩阵上线
...
1. 表格过重，用户不能一眼看到本周内容增长是否健康。
2. “生成下周计划草稿”直接出现在周报页，会越过计划预览和人工确认。
3. 周报需要更多图表和关键指标，而不是所有信息都平铺成表格。
4. 蒸馏词矩阵复盘还没有成为固定模块。

V3 方向：

1. 周报页先展示本周核心指标和趋势，再展示问题。
2. “生成下周计划草稿”改为“进入周计划生成预览”。
3. 下周计划统一在周计划页生成、预览、确认。
4. 新增蒸馏词矩阵复盘：
   - 本周覆盖了哪些蒸馏词
   - 哪些蒸馏词内容类型不完整
   - 哪些蒸馏词 GEO 命中提升
...
6. 发布队列收敛为数据回传。
7. GEO 测试改成诊断复盘台，增加 Prompt 组抽屉和蒸馏词默认全选逻辑。
8. 博客监控改成诊断优先。
9. 周度复盘加入图表、蒸馏词矩阵复盘和跳转周计划生成预览。
...
GEO 测试里的“是否引用官网”必须拆成四层，因为不同层级对业务的意义不同。
...
┌──────────────────────────────────────────────────────────────────────┐
│ 首页数据看板                                      [刷新数据] [去今日发布] │
│ 本周内容生产、官网监控、GEO 诊断和数据回传的总览。                       │
└──────────────────────────────────────────────────────────────────────┘

┌────────────┬────────────┬────────────┬────────────┬────────────┐
│ 本周计划     │ 已生成       │ 已发布       │ 待回填 URL  │ 待数据回传   │
│ 24         │ 16         │ 11         │ 3          │ 8          │
└────────────┴────────────┴────────────┴────────────┴────────────┘
...
┌──────────────────────────────────────────────────────────────────────┐
│ 工作台流程结构                                                         │
│ 知识库 -> 周计划 -> 今日发布 -> URL 回填 -> 数据回传 -> 周报 -> 下周计划 │
│             ↑                GEO 测试 / 博客监控 / 候选池              │
└──────────────────────────────────────────────────────────────────────┘
```
...
┌──────────────────────────────────────────────────────────────────────┐
│ 今日发布                                                 [批量生成正文] │
│ 从已确认周计划中选择今天要生成的文章，生成后进入单篇草稿预览。             │
└──────────────────────────────────────────────────────────────────────┘
...
┌──────────────────────────────────────────────────────────────────────┐
│ 周度复盘                                            [进入周计划生成预览] │
│ 用本周数据解释问题，把信号带到下周计划，不在这里直接生成计划。              │
└──────────────────────────────────────────────────────────────────────┘
...
┌──────────────────────────────────────────────────────────────────────┐
│ 蒸馏词矩阵复盘                                                          │
│ 蒸馏词 | 内容覆盖 | 类型完整度 | GEO 提升 | 竞品占位 | 下周建议           │
└──────────────────────────────────────────────────────────────────────┘
...
| 周初计划 | 周计划页 | 设置篇数，一键生成计划预览，修改确认 | 合并周报、候选池、渠道、GEO 信号 | 计划只包含标题级任务 |
...
| 数据回传 | 发布队列 / 数据回传页 | 上传渠道数据表 | 匹配 URL 和指标 | 指标进入周报 |
| 监控诊断 | 博客监控 / GEO 测试 | 查看问题、入候选池 | 生成诊断和建议动作 | 问题进入下轮计划 |
| 周度复盘 | 周报页 | 看指标、图表和下周信号 | 生成计划信号，不直接生成正文 | 跳转周计划预览 |
...
5. GEO 测试中蒸馏词默认全选，减少漏测。
6. Prompt 组通过抽屉展示和编辑，不作为主页面大表。
7. 周报页不直接生成下周计划，只跳转到周计划生成预览。
8. 单篇草稿质检未通过时，复制全文禁用。
...
3. 用户可以生成周计划预览，并只看到标题级字段。
4. 用户可以在今日发布页选择任务并批量生成正文。
5. 单篇草稿可以人工修改、AI 二次质检、通过后复制。
6. 发布后可以在今日页确认发布并回填 URL。
7. 发布队列只做数据回传。
8. GEO 测试页能展示诊断摘要、问题类型、引用层级和建议动作。
9. 博客监控页先展示诊断，再展示明细表。
10. 周度复盘页能展示图表和蒸馏词矩阵复盘。

### 15.2 规则验收

1. 内容生成必须经过五类 Prompt / 规则模板。
2. 周计划必须使用四层约束模型。
3. GEO 测试必须使用发布量区间建议频率。
4. 官网引用必须记录四层可观察结果。
5. 蒸馏词默认进入 GEO 测试全选逻辑。
