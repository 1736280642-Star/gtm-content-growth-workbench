...
本文档是在 `content-growth-dashboard-refactor.md` 基础上的 V2 细化方案，用于回答当前关于知识库、RAG、自动化、GEO 诊断、月度计划、草稿预览、发布数据回传、博客监控、蒸馏词矩阵和多模型验证的需求与疑问。
...
5. 运营闭环：让问题能够进入候选池、月度计划、草稿、发布、复盘。
...
你希望月度计划默认自动生成，但背后的规则要明确。

月度计划不能只按“每天几篇、哪些渠道”机械生成。它应该综合：
...
月度计划生成不是一个 Prompt 能解决的，至少需要几类 Prompt 或规则模块。
...
2. 月度计划预览页：计划确认前，可以查看“预计草稿结构”，但不生成完整正文。
...
4. “发布后请确认已发布，并回填 URL，否则月度复盘无法统计完整效果。”
