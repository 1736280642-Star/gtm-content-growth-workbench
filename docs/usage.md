...
当前版本可以作为本地试运行 MVP 使用：月度计划、今日生成、终稿确认、发布队列、博客监控、月度复盘、知识库、配置诊断和真实接入页都已经有页面入口和 API 状态流转。
...
1. 打开首页，查看本月任务、发布、博客和 Pipeline 运行概览。
2. 进入“工作台设置”，确认每月发布天数、每日篇数、渠道、产品和日志模式。
3. 进入“月度计划”，点击“生成月度计划”，确认任务后进入今日生成；尚未生成稿件的任务可以删除。
...
8. 进入“月度复盘”，生成月度复盘，查看下月建议；需要交付给团队时可点击“导出 Markdown”复制月度复盘，需要启动下一轮内容排期时可生成下月计划草稿。
9. 进入“真实接入”，逐项查看 MySQL、AI、XCrawl、日志和调度配置状态。

## 4. 一键 Pipeline

首页的“运行 GTM Pipeline”会串联博客同步、日志导入、渠道指标导入和运行记录保存。
...
1. `smoke:pages` 检查主要页面、只读 API 和月度复盘 Markdown 导出 API 是否可访问。
2. `smoke:interactions` 检查关键按钮的 API、确认弹窗、loading、反馈、配置失败提示、任务确认/删除、月度复盘导出、下月计划草稿生成和刷新合约。
3. `smoke:browser` 使用系统 Chrome 真实点击页面，当前覆盖月度计划 Popconfirm、知识库新增 Modal 和发布 URL 回填 Modal，并验证保存后的 DOM 刷新。
4. `smoke:workflow` 会写入 `data/workbench-state.json`，这是为了验证本地持久化链路，并会检查月度复盘 Markdown 是否可导出、月度复盘建议是否可生成下月计划草稿。
...
3. 缺少真实 AI 配置时，内容生成可以走本地规则 fallback。
