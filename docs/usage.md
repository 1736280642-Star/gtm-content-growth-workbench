...
在 V5 月度页面分支中，月度内容矩阵是后续唯一月度计划真源，完成能力对齐和验收后将替代旧周度规划页面及对应规划流程。V5 生产界面收敛为：`月度内容矩阵 -> 批量生成与人工排程 -> 当日执行 -> 月度复盘`。月度策略包已经嵌入月度内容矩阵；异常拦截和文章级人工排程已经并入批量生成中心；当日执行只保留昨日、今日、明日的发布状态。

月度计划配置从产品表达规则包开始：只有 `active` 且 `monthlyProductionReady` 的规则包可选择，选择后自动带出产品，用户多选现有渠道并填写该产品分组的月度总篇数。策略包按蒸馏词、产品配额和 Evidence Preview 摘要展示，不生成文章标题。当前大部分月度配置、异常、排程和复盘仍是持久化适配或演示界面；批量生成中心已新增一个正式 MySQL 单篇链路，可对批准后的 Pharaoh Command 矩阵项执行真实 Retrieval、Final EvidencePack、正式正文生成和 DraftVersion 读取。
...
1. 进入“问题与关键词池”，查看系统自动聚类、去重和规范化后的问题；高置信度问题自动可用，低置信度问题进入观察，只有主体、关系或安全边界冲突进入待决策。
2. 在关键词库中纠正关联或排除误判。关键词由系统自动维护，不需要逐条审核、手动启用或分配角色。
3. 进入“知识库”创建工作区，只填写名称、知识库重点并导入资料；详情仅显示“资料、系统理解、待处理”。
4. 待处理只包含缺关键资料、公开范围无法判断和无法自动恢复的资料失败；非关键事项只限制受影响的表达，不阻断整个知识库。
5. 进入“配置管理”，集中查看模型服务、文章表达预设、发布连接和前台测试连接。表达预设通过表单和可排序结构模块维护，不填写完整 Prompt。
...
3. 进入“批量生成中心”，查看标题确认、Final Evidence Gate、生成质检、异常分流和人工排程。
