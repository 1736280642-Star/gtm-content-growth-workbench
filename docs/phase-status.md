...
| Phase 7-C-UI | 持续补齐中          | 新增真实接入页，核心表格页已补空状态、失败重试状态和下一步动作，页面级 smoke 已具备；月度复盘、知识库维护、关键操作确认和使用说明已接上     |
...
5. API 试跑通过：月度计划生成、批量生成稿件、终稿确认、发布 URL 回填均已写入 `data/workbench-state.json`。
...
5. 新增 `smoke:pages` 页面级访问检查，可覆盖主要页面、运行态 API、配置诊断 API 和月度复盘 API。
7. 月度复盘页“生成月度复盘”按钮已调用 `GET /api/monthly-reviews/{month}`，并把结果展示回页面。
8. 知识库页已支持新增、编辑、启用/停用，并通过 `smoke:workflow` 验证本地持久化写入。
9. 月度计划生成/重生成、今日批量/单篇生成、稿件重生成/入队、发布标记、GEO 批量测试和人工修正已补二次确认。
...
5. 新增 `smoke:interactions`，覆盖首页 Pipeline、月度计划、今日生成、终稿确认、发布队列、博客监控、GEO、月度复盘、知识库、设置、AI 配置和真实接入页的关键交互合约。
6. AI 配置页和真实接入页已补配置状态加载失败提示与重试，避免把配置接口失败误判为全部待配置或全部就绪。
7. 博客同步、博客数据导入、AI Bot 日志导入、渠道指标导入已补二次确认。
8. 月度计划页已支持单条确认、批量确认和删除未生成任务，并接入 `POST /api/content-tasks/confirm` 与 `DELETE /api/content-tasks/{id}`。
...
15. 月度复盘页已支持按发布状态、博客 GEO 结果和 GEO 执行状态筛选复盘明细，并在筛选无结果时提供清空筛选动作。
16. 月度计划页已支持按任务状态、渠道和产品筛选任务，并在筛选无结果时提供清空筛选动作。
...
23. 月度计划页已补稿件承接、发布承接和下一步判断。

2026-06-18 月度复盘 Markdown 导出补齐验证结果：

1. 新增 `GET /api/monthly-reviews/{month}/export`，可返回结构化月度复盘 Markdown。
2. 月度复盘页新增“导出 Markdown”，会调用导出 API 并复制到剪贴板。
4. `smoke:pages` 已覆盖月度复盘 Markdown 导出 API。
5. `smoke:interactions` 已覆盖月度复盘导出的 API、剪贴板、loading 和反馈合约。
6. `smoke:workflow` 已覆盖月度复盘 Markdown 导出主链路。

2026-06-18 月度复盘建议生成下月计划草稿补齐验证结果：

1. 新增 `POST /api/monthly-reviews/{month}/next-plan`，可把月度复盘建议转成下一月度计划草稿。
2. 月度复盘页新增“生成下月计划草稿”，并通过 `Popconfirm` 做二次确认。
3. 生成草稿会复用工作台默认发布天数、每日篇数、渠道和产品配置。
4. 生成结果会写入本地运行态，并刷新页面数据；月度计划页可直接查看新草稿。
5. `smoke:workflow` 已覆盖月度复盘建议生成下月计划草稿主链路。
...
1. 新增 `POST /api/blog-articles/{id}/candidate/task`，可从博客候选主题生成当前月度计划下的渠道补强任务。
2. 生成任务会追加到当前 `monthlyPlan`，不覆盖已有任务、稿件或发布队列。
3. 博客候选池页新增“生成任务”动作，并通过 `Popconfirm` 做二次确认。
4. 生成后候选主题会标记为 `planned`，月度计划页可继续编辑、确认和生成稿件。
...
11. 月度计划、今日任务、终稿、发布队列、GEO、博客监控和博客候选池已经从静态按钮推进到真实 API 调用。
...
29. 月度计划页已支持确认任务进入今日任务生成队列，并允许删除尚未生成稿件的计划任务。
30. 月度复盘已支持 Markdown 导出，可作为内部月度复盘交付物复制使用。
31. 月度复盘已支持把月度复盘建议生成下月计划草稿，补齐 Phase 6 复盘到计划的反哺闭环。
33. 博客候选池页已支持把候选主题标记为已规划或移出候选池，补齐候选主题后续处置闭环。
34. 发布队列页已支持单条录入渠道表现指标，补齐 Phase 4.7。
35. 博客候选池页已支持从候选主题生成渠道补强任务，补齐 Phase 5.6。
37. 新增 `smoke:browser`，通过系统 Chrome 真实点击月度计划 Popconfirm、知识库新增 Modal 和发布 URL 回填 Modal，并验证刷新后的 DOM 状态。
...
42. 月度复盘页已支持按发布状态、博客 GEO 结果和 GEO 执行状态筛选复盘明细，补齐 Phase 6.3、6.4、6.5 的问题定位视角。
43. 月度计划页已支持按任务状态、渠道和产品筛选任务，补齐 Phase 2.2 的计划管理视角。
...
50. 月度计划页已展示稿件承接、发布承接和下一步判断，补齐 Phase 2.2 的任务下游承接视角。
51. 首页已展示执行队列，按月度计划确认、稿件生成、终稿处理、发布回填、博客处置、GEO 处置和月度复盘汇总当前阻塞项，并提供对应页面跳转，补齐负责人从首页直接判断下一步动作的视角。
...
56. 博客候选池页已展示候选处置判断和处理动作，能区分确认入池、生成任务、标记规划、复查来源和已规划回看，补齐从候选清单到月度计划承接的执行视角。
57. 月度复盘页已展示复盘行动队列，能从发布队列、URL 回填、渠道指标、博客候选、GEO 配置和 GEO 缺口中归纳优先动作，并给出对应可执行入口，补齐从月度复盘到下月执行的承接视角。
58. 今日任务页已展示执行承接判断，能从计划确认、稿件生成、生成排查、质检处理、终稿确认、发布承接、URL 回填、指标录入和月度复盘中给出下一步动作与入口，补齐从当天任务到下游流程的执行视角。
59. 终稿确认页已展示执行承接判断，能从生成配置、重新生成、质检阻断、发布队列、人工发布、URL 回填、指标录入和月度复盘中给出下一步动作与入口，补齐从终稿确认到发布复盘的执行视角。
60. 博客监控页已展示处理动作与可执行入口，能从诊断、加入候选池、候选池处理、已规划回看、继续观察和月度复盘中给出下一步动作与跳转，补齐从博客诊断到候选承接的执行视角。
62. 发布队列页已展示处理动作与可执行入口，能从人工发布、URL 回填、指标录入、失败重导出和月度复盘中给出当前唯一优先动作，并通过浏览器 smoke 验证“标记已发布 -> 回填 URL”真实顺序。
...
2026-06-19 月度计划、首页与月度复盘闭环表达补齐后的最新验证结果：
1. 月度计划页已拆分 `下一步 / 处理动作 / 可执行入口 / 维护`，计划确认、稿件生成、AI 配置排查、终稿处理、发布回填、指标录入和月度复盘都有明确入口。
2. 首页执行队列已显式展示 `当前状态 / 处理动作 / 可执行入口`，负责人可以从首页直接判断当前阻塞项和下一步跳转位置。
3. 月度复盘行动队列已统一把 `当前动作` 改为 `处理动作`，全站关键页面表达收敛为 `当前状态 -> 下一步 -> 处理动作 -> 可执行入口`。
...
2026-06-19 月度复盘建议与真实接入顺序闭环收口后的最新验证结果：
1. 月度复盘页的 `下月建议` 已从纯文本列表升级为 `建议 / 下一步 / 处理动作 / 可执行入口`，未生成月度复盘时会先引导生成月度复盘，已生成月度复盘时会把建议复核和下月计划草稿承接拆开。
...
1. 首页 `执行队列` 已从列表说明升级为 `事项 / 当前状态 / 数量 / 下一步 / 处理动作 / 可执行入口`，负责人不需要读长句也能判断先处理月度计划、稿件、发布、博客、GEO 还是月度复盘。
3. 首页 `Pipeline 运行记录` 已补 `下一步 / 处理动作 / 可执行入口`，成功记录进入月度复盘，部分完成或失败记录回到真实接入页排查。

2026-06-19 生产模式运行态闭环修复后的最新验证结果：
1. 知识库页已补稳定排序，新增知识库会按最近同步时间和创建时间优先展示，保存后可以直接在当前页看到新记录。
2. `GET /api/workbench-state`、`/api/dashboard/summary`、`/api/runtime-config/status`、`/api/config-diagnostics`、`/api/knowledge-bases`、`/api/workspace-settings`、`/api/pipeline/runs/export`、`/api/monthly-reviews/{month}`、`/api/monthly-reviews/{month}/export` 已统一声明 `force-dynamic`，避免 `next start` 生产模式返回构建时旧快照。
...
2. `POST /api/monthly-plans/generate`
3. `PATCH /api/monthly-plans/{id}`
...
26. `GET /api/monthly-reviews/{month}`
27. `GET /api/monthly-reviews/{month}/export`
28. `POST /api/monthly-reviews/{month}/next-plan`
