...
| 目标 | 说明 |
|---|---|
| 1. 让月度计划可配置 | 支持按周生成选题任务，每天发布量、渠道分布可手动调整 |
...
```text
+------------------------------------------------------+
| 月度计划  2026-06-16 ~ 2026-06-22       [生成选题]     |
+------------------------------------------------------+
| 发布设置                                             |
| 每月发布天数 [5]  默认每天篇数 [3]  渠道 [多选]     |
...
```mermaid
erDiagram
    WORKSPACE_SETTING ||--o{ MONTHLY_PLAN : configures
    MONTHLY_PLAN ||--o{ CONTENT_TASK : contains
    ...
    MONTHLY_PLAN ||--o| MONTHLY_REVIEW : summarized_by
```
...
|---|---|---|
| id | 设置 ID | 是 |
| default_publish_days | 默认每月发布天数 | 是 |
...
| `POST /monthly-plans/generate` | 生成周选题计划 |
| `PATCH /monthly-plans/{id}` | 调整月度计划配置 |
...
| `GET /monthly-reviews/{month}` | 获取月度复盘 |
...
| 模块 | 验收标准 |
|---|---|
| 首页 | 能看到本月度计划、今日任务、待确认、待发布、博客诊断提醒 |
| 月度计划 | 能按设置生成一周渠道任务，并支持人工调整 |
| 每日任务 | 能批量生成当天文章 |
