...
```text
GEO_RESEARCH_OPENAI_API_KEY
OPENAI_API_KEY
GEO_RESEARCH_OPENAI_MODEL
GEO_RESEARCH_OPENAI_BASE_URL
```

`GEO_RESEARCH_OPENAI_API_KEY` 与 `OPENAI_API_KEY` 二选一；`GEO_RESEARCH_OPENAI_MODEL` 必填。未配置时任务进入 `pending_config`，不会降级。
