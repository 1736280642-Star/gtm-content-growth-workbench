...
3. 自动化设置不放在内容生成页，而放在“知识库资料更新 / 博客监控 / GEO 测试”这些业务页面里。
...
> 虽然用户分别在知识库页、博客监控页、GEO 测试页点不同的自动化功能，但系统内部其实使用的是同一种定时任务机制。  
...
“每周 15 篇以上才每周测，否则一个月测一次”不够合理。

因为每周 15 篇已经意味着月度内容变化很大，一个月才测一次会导致补强动作滞后。
...
```mermaid
flowchart TD
  A["周报问题 / 候选池 / 渠道数据 / GEO问题"] --> B["系统生成周计划预览"]
  B --> C["用户确认或修改计划"]
  C --> D["进入今日发布页"]
...
4. 发布队列只负责数据回传
5. 博客监控与 GEO 测试都应优先展示“问题与动作”，而不是先展示大表格
