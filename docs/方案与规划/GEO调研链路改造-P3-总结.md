...
- 规则包的 `activatedBy` 必须非空、`activatedAt` 必须为有效时间；只有同时满足结构与激活元数据约束才视为“已激活”；
...
- `startGeoResearchRun` 与 `createGeoResearchRunRecord` 在创建事务前再次执行同一覆盖校验，直接调用启动 API 也无法绕过就绪检查；
...
- 规则包校验同时发生在就绪检查（read 路径）和 run 创建（service + repository 写路径）；规则包失效或目标渠道不覆盖时均拒绝建 run。
...
- 规则包当前为环境变量驱动（`GEO_CHANNEL_RULE_PACK_JSON`），后续可切换治理存储表；环境变量内容现已强制携带 `activatedBy/activatedAt`，但人工激活动作与审计记录仍需治理存储表承接；
