...
默认扩量门槛如下，可通过 `PUBLISH_RELIABILITY_*` 环境变量调整，但不得在缺少 24h/72h 证据时把空值视为通过：

- 每个平台至少 3 个真实已受理样本，且必须来自 3 个不同 `draftId`；同一草稿的重复排程不能增加样本数。
- 提交接受率、公开转换率不低于 90%。
- 24h、72h 存活率不低于 95%。
- 风险阻断率不高于 10%。
- 真实重复发布数必须为 0。
...
| Phase 1 | 完成 | 页面上下文提交、三平台响应/Toast 脱敏捕获、平台串行、Runner 单实例、浏览器冷启动重试、动态节点重取、平台级风险写门禁 | 扩展方案保留为可选执行器；当前主链路使用 Runner |
| Phase 2 | 完成 | Publish Job REST API、官方 MCP SDK Server、8 个高层发布 Tool；长任务异步入队并返回 durable handle | 生产部署时补远程安全传输和权限模型 |
...
工程收口已补齐：按平台独立结果与 URL、公开实体及其 worker 核验请求按文章 ID/规范化 URL 去重、发布写操作串行化、本地 JSON 原子替换、待核验分段退避、公开 URL 里程碑优先级、24h 与 72h 精确调度、稳定任务显式复验门槛、风险阻断率与真实重复发布率、平台级风险/认证写门禁，以及掘金最多一次的不可变平台变体改写。MCP 的发布与 reconciliation Tool 只执行异步入队并返回同一 Publish Job handle，不再同步等待浏览器；确定性 worker 是唯一长任务执行者。三平台统一捕获发布响应与可见 Toast，但不保存请求头、Cookie 或完整响应正文，最终仍以创作后台和公开 URL 为准。只读核验允许对已知瞬时浏览器断连执行一次浏览器重建重试，正式发布动作不共享该重试策略。Reliability API 将工程契约与扩量就绪分开返回；缺少样本或自然存活证据时 `rolloutReady` 必须为 `false`。相关契约验证为 Runner `44/44`、正式发布 `32/32`、MCP `1/1`、结构校验 `200/200`。这些能力解决的是执行一致性与可观测性，不能替代平台审核，也不会把掘金当前未公开的结果升级为成功。
