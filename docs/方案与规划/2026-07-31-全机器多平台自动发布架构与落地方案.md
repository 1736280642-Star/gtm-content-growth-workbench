...
工程收口已补齐：按平台独立结果与 URL、发布写操作串行化、本地 JSON 原子替换、待核验分段退避、24/72 小时阈值精确调度、可靠性指标口径修正，以及掘金最多一次的不可变平台变体改写。相关契约验证为 Runner `40/40`、正式发布 `25/25`、MCP `1/1`、结构校验 `200/200`。这些能力解决的是执行一致性与可观测性，不能替代平台审核，也不会把掘金当前未公开的结果升级为成功。
