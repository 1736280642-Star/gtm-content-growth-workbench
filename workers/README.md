...
npm.cmd run worker:run-geo-tests -- --base-url http://127.0.0.1:3000 --platforms 通义千问,DeepSeek --prompt "推荐几家国内 Dify 企业版服务商"
...
npm.cmd run worker:run-pipeline -- --base-url http://127.0.0.1:3000 --skip-blog --log-file-path data/demo-ai-bot-log.csv --channel-metrics-path imports/channel-metrics-smoke.csv --geo-platforms 通义千问,DeepSeek
...
2. `run-geo-tests.mjs`: 调用 `/api/geo-tests/run`，支持 `platforms`、`prompt`、`prompt-file`。
3. `import-demo-log.mjs`: 调用 `/api/log-imports`，支持 `sourceType`、`filePath`、`sourcePath`、`csv`、`raw-log`。
4. `import-channel-metrics.mjs`: 调用 `/api/channel-metrics/import`，支持 `filePath`、`sourcePath`、`csv`。
5. `run-pipeline.mjs`: 串联博客同步、日志导入、渠道数据导入、GEO 测试和周报读取，支持 `skip-*` 参数跳过外部依赖步骤。
6. `schedule-pipeline.mjs`: 按固定间隔重复调用 `/api/pipeline/run`，默认只执行一次；传 `--repeat` 才会循环。
