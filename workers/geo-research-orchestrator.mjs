...
const [{ runAutomaticGeoResearchOrchestration }, { getV5GovernancePool }] = await Promise.all([
...
try {
  const result = await runAutomaticGeoResearchOrchestration({ actor });
  console.log(JSON.stringify({ status: "completed", ...result }));
  if (result.products.some((item) => item.status === "waiting_for_sources")) process.exitCode = 2;
} catch (error) {
