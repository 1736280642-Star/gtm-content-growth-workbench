...
if (args.help || args.h) {
  printJson({
    worker: "run-pipeline",
    usage:
      "node workers/run-pipeline.mjs [--base-url URL] [--skip-blog] [--skip-log] [--skip-channel-metrics] [--month YYYY-MM] [--blog-source-url URL] [--blog-source-urls URL1,URL2] [--blog-source-path PATH] [--blog-json TEXT] [--log-file-path PATH] [--log-source-type demo_csv|nginx_log|cdn_log] [--channel-metrics-path PATH] [--channel-metrics-csv TEXT]"
  });
  process.exit(0);
}
...
try {
  ...
  const monthlyReview = await getJson(baseUrl, `/api/v5/monthly-reviews/${month}`);
  ...
  printJson({
    ...
    steps,
    monthlyReview: monthlyReview.body,
