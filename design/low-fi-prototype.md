...
4. 内容监控塔如何汇总官网监控、AI 访问数据，并为 MonthlyReview 提供证据。
...
| P1 | 内容监控塔 | 汇总监控证据与 MonthlyReview 结果 |
| P1 | 知识库 | 管理内容生成依据 |
...
+----------------------------+-------------------------+
| 官网博客诊断               | AI 访问概览             |
| SEO 问题：12               | AI Bot PV：128 [Demo]   |
| GEO 诊断待处理：5          | Top Paths：8 [Demo]     |
+----------------------------+-------------------------+
| 需要处理                                                    |
| 1. 6 月 17 日还有 2 篇文章未确认                            |
...
月度内容矩阵页用于一次性确定一个自然月的内容任务。用户可以自动生成，也可以手动调整。

### 5.2 线框图

```text
+------------------------------------------------------+
| 月度内容矩阵  2026-06-01 ~ 2026-06-30 [生成月度方案] |
+------------------------------------------------------+
| 发布设置                                                |
| 本月目标篇数：[15]                                     |
| 排程策略：[分散排程]                                   |
...
| 06-17 | 公众号 | JOTO | ... | 品牌 | 待确认 | 编辑/重生成 |
| 06-17 | CSDN | 唯客 | ... | 技术 | 已确认 | 查看        |
| 06-18 | 知乎/头条 | 唯客 | ... | FAQ | 待确认 | 编辑/删除  |
+------------------------------------------------------+
| [批量确认] [重生成未确认] [保存为草稿] [确认月度矩阵]   |
+------------------------------------------------------+
```
...
| 状态 | 含义 | 可执行操作 |
|---|---|---|
| 草稿 | 月度矩阵未确认 | 编辑、重生成、删除 |
| 待确认 | 系统已生成但人工未确认 | 编辑、确认、重生成、删除 |
| 已确认 | 可进入每日任务 | 查看、少量编辑 |
| 执行中 | 已有任务进入生成流程 | 不建议大规模修改 |
| 已完成 | 本月计划结束 | 查看内容监控塔与 MonthlyReview |
...
官网博客监控页用于诊断官网博客内容资产，不负责博客创作。它输出三类结果：SEO 问题、GEO 诊断问题、博客候选池。
...
| 标签 | 页面展示 | 含义 |
|---|---|---|
| 真实 | 绿色标签 | 来自 XCrawl、渠道接口、人工回填等真实执行 |
...
| 博客候选池                            [导出候选清单] |
+------------------------------------------------------+
| 候选来源：[SEO诊断] [博客诊断] [渠道表现] [人工新增] |
+------------------------------------------------------+
| 候选列表                                                |
| 标题建议 | 来源 | 原因 | 优先级 | 状态 | 操作          |
| ... | 博客诊断 | 官网引用线索不足 | 高 | 待接入 | 查看 |
...
内容监控塔用于汇总渠道表现、官网审计和 AI 可见性数据，并展示 MonthlyReview 形成的下月候选建议。

### 12.2 线框图

```text
+------------------------------------------------------+
| 内容监控塔  2026-06-01 ~ 2026-06-30        [刷新数据]   |
+------------------------------------------------------+
| 管理层摘要                                              |
...
| 通义千问 | 内容生成 | ... | 启用 | 编辑/测试连接       |
| DeepSeek | 内容生成 | ... | 启用 | 编辑/测试连接       |
| 豆包     | 内容生成 | ... | 启用 | 编辑/测试连接       |
+------------------------------------------------------+
| Prompt 配置                                             |
| 名称 | 类型 | 渠道 | 状态 | 操作                       |
